...
## EDIT THESE SETTINGS ##
PLEXPY_APIKEY = '61e9971e07404edf9fa740b248ba12bd'  # Your PlexPy API key
...
def get_get_metadata(rating_key):
    ...
    try:
        ...
        res_data = response['response']['data']
        res_data['file'] = res_data['media_info'][0]['parts'][0]['file']
        res_data['file_size'] = res_data['media_info'][0]['parts'][0]['file_size']
        result = METAINFO(data=res_data)
        return result
...
def get_all_library_media_info(section_id):
    # Get the data on the PlexPy media info tables.
    payload = {'apikey': PLEXPY_APIKEY,
               'section_id': section_id,
               'cmd': 'get_library_media_info',
...
for i in glt:
    try:
        gglm = get_all_library_media_info(i)
        for x in gglm:
            try:
    ...
    try:
        ...
        if x.grandparent_title == '' or x.media_type == 'movie':
            ...
            print(u"{x.title} ({x.rating_key}) was added {when} and has not been"
                  u"watched. [{x.file}]".format(x=x, when=added))
        else:
            # Shows
            print(u"{x.grandparent_title}: {x.title} ({x.rating_key}) was added {when} and has"
                  u"not been watched. [{x.file}]".format(x=x, when=added))
        path_lst += [x.file]
